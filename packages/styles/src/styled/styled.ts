...
const styled: Styled = ( tag, options = {} ) => {
	// @ts-ignore
	const { name, slot, shouldForwardProp: customShouldForwardProp, ...styledOptions } = options;

	const displayName = ( process.env.NODE_ENV !== 'production' && name ) ? `${ name }${ slot || '' }` : undefined;

	const defaultResolver = emotionStyled(
		tag as any,
		{
			shouldForwardProp: ( prop: string ) => {
				if ( customShouldForwardProp ) {
					return shouldForwardProp( prop ) && customShouldForwardProp( prop );
				}
